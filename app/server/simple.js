...
var server = http.createServer(function (req, res) {
    ...
    if (request.url in routingTable) {
        ...
        var requestTable = request.url.split('/')

        // Is a Static File
        if (requestTable[1] === ViewEngine.areas.client.content) {

            // Log Content File
            log("Content File:" + request.url);
            ...
            if (isStaticFile(request.url)) {
                ...
                log("Static File");
                // Open Static File
                openFileStatic(request);
            } else {

                // Log File Page Not Found
                log(request.url + " [ Not Found ]");
                pageNotFound();
            };
        }
        else {
